...
class QWenModel(QWenPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.json_wte = nn.Embedding(self.vocab_size, self.embed_dim) # 新增对json format表格数据的编码器
    ...
    def forward(
        ...
        if torch.any(input_ids == 151646) and torch.any(input_ids == 151647) : # 表格数据用特殊字符包裹
        ...
        if json_datas is not None:
            for idx, (i, a, b) in enumerate(json_pos):
                hidden_states[i][a + 1 : b] = json_datas[idx][:b-a-1] # 只赋值中间227个，特殊字符依然保留原来的训练方式
