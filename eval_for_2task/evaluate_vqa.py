...
ds_collections = {
    'vqarad_slack': {
        'train': 'data/VQARAD_SLAKE/dataset_train.jsonl',
        'test': 'data/VQARAD_SLAKE/dataset_eval.jsonl',
        'metric': 'accuracy',
        'max_new_tokens': 10,
    },
    'vqav2_val': {
...
def relaxed_correctness(target: str,
    ...
    if prediction_float is not None and target_float: # 处理数字
        relative_change = abs(prediction_float -
                              target_float) / abs(target_float)
        return relative_change <= max_relative_change
    else:
        return prediction.lower() == target.lower() # 绝对相等才可以
...
def evaluate_exact_match_accuracy(entries):
    ...
    for elem in entries:
        ...
        score = max([
            (1.0 if
             (elem['answer'].strip().lower() == ann.strip().lower() or elem['answer'].strip().lower() in ann.strip().lower() or ann.strip().lower() in elem['answer'].strip().lower()) else 0.0)
            for ann in elem['annotation']
        ])
        scores.append(score)
    return sum(scores) / len(scores)
...
def collate_fn(batches, tokenizer):
    ...
    annotations = [_['annotation'] for _ in batches] # answer
...
class VQADataset(torch.utils.data.Dataset):
    ...
    def __init__(self, train, test, prompt, few_shot):
        ...
        self.prompt = prompt #     prompt = '<img>{}</img>{} Answer:' # 测试时只有一张图片
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(image, str):
            ...
            if self.few_shot > 0:
                ...
                for sample in few_shot_samples:
                    ...
                    if isinstance(sample['image'], str):
                        ...
                        #     sample['image'],
                        #     sample['question']) + f" {sample['answer']}"
                        for n, i in enumerate(sample['image']):
                            few_shot_prompt += f'Picture {n+1}: <img>{i}</img>\n'
                        few_shot_prompt += sample['question'] + f" Answer: {sample['answer']}"
            ...
            return {
                'question': few_shot_prompt + value,
                'question_id': question_id,
                'annotation': annotation
            }
...
if __name__ == '__main__':
    ...
    prompt = '<img>{}</img>{} Answer:' # 测试时只有一张图片
    ...
    for _, (question_ids, input_ids, attention_mask,
        ...
        # The above code is a Python comment section. Comments in Python start with a hash symbol (#)
        # and are used to provide explanations or notes within the code. Comments are ignored by the
        # Python interpreter and are not executed as part of the program.
        answers = [
        ...
        for question_id, answer, annotation in zip(question_ids, answers, annotations):
            ...
            elif args.dataset in ['vqarad_slack','docvqa_val', 'infographicsvqa', 'gqa_testdev', 'ocrvqa_val', 'ocrvqa_test']:
                outputs.append({
    ...
    if torch.distributed.get_rank() == 0:
        ...
        json.dump(merged_outputs, open(results_file, 'w'), ensure_ascii=False, indent=4)
