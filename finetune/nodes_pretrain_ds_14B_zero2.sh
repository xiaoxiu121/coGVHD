...
ENV_PATH=/nfs/llm/anaconda3/envs/qwenvl1/bin
...
HOST_FILE_PATH="cll_scripts/hostfile"

MODEL="/nfs/llm/caolili/Qwen-VL/hf_models/Qwen-14B-VL-Chat-zero"
...
run_cmd="${OPTIONS_NCCL} ${ENV_PATH}/deepspeed --num_nodes ${NUM_WORKERS} --num_gpus ${NUM_GPUS_PER_WORKER} --master_port ${MASTER_PORT} --hostfile ${HOST_FILE_PATH} finetune.py \
    --model_name_or_path $MODEL \
    --data_path $DATA $DATA1 $DATA2 $DATA3 $DATA4 $DATA5 $DATA6 $DATA7 $DATA8 $DATA9 $DATA10 \
    --bf16 True \
    --fix_vit True \
    --fix_llm False \
