        ...
        for j, sentence in enumerate(source):
            ...
            if role == '<|im_start|>user':
                ...
                _target = [im_start] + [IGNORE_TOKEN_ID] * len(tokenizer(role).input_ids) + \
                    _input_id[len(tokenizer(role).input_ids)+1:-2] + [im_end] + nl_tokens # target中的value答案没有向后移动一位
            else:
                raise NotImplementedError
            target += _target
        assert len(input_id) == len(target)
...
def make_supervised_data_module(
    ...
    if isinstance(data_args.data_path,list):
        for i in data_args.data_path:
            train_json = json.load(open(i, "r"))
...
def collate_fn(batch):
    ...
    attention_mask = []
    for sample in batch:
        sample_input_ids = sample['input_ids'].tolist() + [151643] * (max_len - len(sample['input_ids']))
        input_ids.append(sample_input_ids)
        labels.append(sample['labels'].tolist() + [IGNORE_TOKEN_ID] * (max_len - len(sample['labels'])))
        attention_mask.append(torch.tensor(sample_input_ids).ne(151643).unsqueeze(0))
...
def train():
    ...
    if not training_args.use_lora:
        ...
        if training_args.fix_wte:
            model.transformer.wte.requires_grad_(False)
        if training_args.fix_json_wte:
            model.transformer.json_wte.requires_grad_(False)
